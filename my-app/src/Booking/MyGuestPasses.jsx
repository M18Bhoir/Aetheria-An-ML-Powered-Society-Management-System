...
import { HiOutlinePlusCircle, HiClock } from "react-icons/hi";
...
function MyGuestPasses() {
  ...
  const fetchMyPasses = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Could not load your guest passes.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-4 md:p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-4xl mx-auto">
      ...
      {!loading && !error && passes.length === 0 && (
        <p className="text-center text-gray-500 dark:text-gray-400">
          You haven't requested any guest passes yet.
        </p>
      )}
      ...
      {!loading && !error && passes.length > 0 && (
        <div className="space-y-4">
          {passes.map((pass) => (
            <div
              key={pass._id}
              className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-700 shadow-sm"
            >
              ...
              {(pass.arrivalTime || pass.departureTime) && (
                <div className="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-md mb-3 border border-blue-200 dark:border-blue-700 flex items-start gap-4">
                  <div className="flex items-start gap-2">
                    <HiClock className="text-blue-600 dark:text-blue-400 mt-0.5 flex-shrink-0" />
                    <div>
                   ...
                   {(pass.checkInTime || pass.checkOutTime) && (
                     <div className="border-l border-blue-200 dark:border-blue-700 pl-4 flex flex-col gap-1">
                        {pass.checkInTime && (
                          <div className="flex items-center gap-2">
                             <span className="text-[10px] font-bold text-green-600 dark:text-green-400 uppercase w-12">Arrived:</span>
                             <span className="text-xs font-mono text-gray-700 dark:text-gray-300">
                               {new Date(pass.checkInTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                             </span>
                          </div>
                        )}
                        {pass.checkOutTime && (
                          <div className="flex items-center gap-2">
                             <span className="text-[10px] font-bold text-rose-600 dark:text-rose-400 uppercase w-12">Left:</span>
                             <span className="text-xs font-mono text-gray-700 dark:text-gray-300">
                               {new Date(pass.checkOutTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                             </span>
                          </div>
                        )}
                     </div>
              ...
              {pass.status === "Approved" && (
                <div className="p-2 bg-blue-100 dark:bg-blue-900 rounded my-2">
                  ...
                  <span className="text-lg font-bold text-blue-700 dark:text-blue-300 ml-2">
                    {pass.code}
                  </span>
                </div>
              )}
