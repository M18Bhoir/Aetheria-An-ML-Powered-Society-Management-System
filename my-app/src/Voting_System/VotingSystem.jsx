    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-2">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={() => navigate(isAdmin ? "/admin/create-poll" : "/dashboard/create-poll")}
      ...
      {error && (
        <motion.div variants={itemVariants} className="text-center p-8 glass-dark border border-rose-500/20 rounded-3xl text-rose-300 shadow-2xl">
      ...
      {!loading && !error && polls.length === 0 && (
        <motion.div variants={itemVariants} className="flex flex-col items-center justify-center p-20 glass-card text-gray-500">
          <Vote size={64} className="mb-6 opacity-10" />
          <p className="text-lg font-bold tracking-tight">No active polls found.</p>
        </motion.div>
      )}
        ...
        {!loading &&
          polls.map((poll) => {
            ...
            return (
              ...
              >
                <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-10 transition-opacity pointer-events-none group-hover:scale-110 transition-transform duration-700">
                ...
                <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-center relative z-10">
                  <p className="text-xs font-bold text-gray-500 tracking-widest uppercase">
                    Total: <span className="text-white ml-1">{totalVotes} VOTES</span>
                  </p>
                  <button
                    onClick={() => navigate(`/dashboard/poll/${poll._id}`)}
...
export function PollDetail() {
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      const res = await api.get(isAdmin ? `/api/polls/admin/${id}` : `/api/polls/${id}`);
      setPoll(res.data);
    } catch (err) {
      setVoteMessage({ type: "error", text: err.response?.data?.msg || "Vote failed." });
    }
  };

  if (loading) return <div className="text-center text-gray-400 py-32 animate-pulse font-bold uppercase tracking-widest">Loading...</div>;
  ...
  return (
    ...
    >
      <button
        onClick={() => navigate(isAdmin ? "/admin/manage-polls" : "/dashboard/voting")}
      ...
      <div className="glass-card p-10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] relative overflow-hidden">
        ...
        <div className="mb-10 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-2.5 px-4 py-2 bg-white/5 rounded-2xl border border-white/10">
              ...
              <span className="text-sm font-bold text-gray-300">{poll.createdBy?.name || "Admin"}</span>
            </div>
            <div className="flex items-center gap-2.5 px-4 py-2 bg-white/5 rounded-2xl border border-white/10">
              <Clock size={16} className="text-blue-400" />
              <span className="text-sm font-bold text-gray-300">{new Date(poll.createdAt).toLocaleDateString()}</span>
            </div>
          </div>
        </div>
          ...
          {voteMessage && (
            <motion.div
              ...
              className={`flex items-center p-5 rounded-2xl mb-8 text-sm font-black uppercase tracking-wide border overflow-hidden ${
                voteMessage.type === "error" ? "bg-rose-500/10 border-rose-500/20 text-rose-400" : "bg-emerald-500/10 border-emerald-500/20 text-emerald-400"
              }`}
            >
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <div className="space-y-4">
            <h3 className="text-xs font-black text-gray-500 uppercase tracking-widest mb-2 px-2">Select Your Option</h3>
            {poll.options.map((option, index) => (
              <label
                key={index}
                className={`flex items-center gap-4 p-5 rounded-2xl cursor-pointer border transition-all duration-300 group
              ...
              >
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${selectedOptionIndex === index ? "border-blue-400 scale-110" : "border-gray-500 group-hover:border-gray-300"}`}>
                  {selectedOptionIndex === index && <motion.div layoutId="dot" className="w-3 h-3 bg-blue-400 rounded-full shadow-[0_0_10px_rgba(96,165,250,0.8)]"></motion.div>}
                </div>
                <input type="radio" value={index} checked={selectedOptionIndex === index} onChange={() => setSelectedOptionIndex(index)} className="hidden" />
                <span className={`font-bold tracking-tight text-lg ${selectedOptionIndex === index ? "text-white" : "text-gray-400 group-hover:text-gray-200"}`}>{option.text}</span>
              </label>
            ))}
            <button onClick={handleVote} className="w-full mt-6 py-5 rounded-2xl bg-blue-600 text-white font-black uppercase tracking-widest shadow-2xl shadow-blue-500/20 hover:bg-blue-500 hover:-translate-y-1 transition-all flex items-center justify-center gap-3 active:scale-95">
                ...
                return (
                  <div key={index}>
                    <div className="flex justify-between items-end mb-2 px-1">
                      <span className="text-sm font-bold text-gray-300">{option.text}</span>
                      <span className="text-[10px] font-black text-blue-400 bg-blue-400/10 px-2 py-1 rounded-lg uppercase">{percentage}%</span>
                    </div>
                    ...
                    <p className="text-[10px] text-gray-600 font-bold mt-1.5 px-1 uppercase tracking-tighter">{option.votes} total votes</p>
                  </div>
                );
              })}
            </div>
            <div className="mt-10 pt-6 border-t border-white/5 flex flex-col items-center">
              <span className="text-[10px] font-black uppercase text-gray-500 tracking-widest mb-1">Total Participation</span>
