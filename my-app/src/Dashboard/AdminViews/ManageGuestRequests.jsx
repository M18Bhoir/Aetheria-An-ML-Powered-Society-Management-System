  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-lg shadow-lg">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Manage Guest Pass Requests</h2>

        {loading && <p className="text-gray-500 dark:text-gray-400">Loading requests...</p>}
        {error && <p className="text-red-500 dark:text-red-300">{error}</p>}
        
        {!loading && !error && passes.length === 0 && (
           <p className="text-center text-gray-500 dark:text-gray-400">No guest pass requests found.</p>
        )}
        ...
        {!loading && !error && passes.length > 0 && (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {passes.map((pass) => (
                  <tr key={pass._id} className="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 align-top">
                    <td className="py-3 p-2 font-medium text-gray-800 dark:text-gray-200">{pass.guestName}</td>
                    <td className="py-3 p-2 text-gray-600 dark:text-gray-300">{pass.requestedBy?.name} ({pass.requestedBy?.userId})</td>
                    ...
                    <td className="py-3 p-2 font-mono text-blue-600 dark:text-blue-400">{pass.code || 'N/A'}</td>
                      ...
                      {pass.status === 'Pending' && (
                        <div className="flex justify-center space-x-2">
                          <button 
                            ...
                            className="p-1 text-green-600 hover:text-green-800 disabled:opacity-50" title="Approve">
                            <Check size={18} />
                          </button>
                          <button 
                            onClick={() => handleReject(pass._id)}
                            disabled={updating === pass._id}
                            className="p-1 text-red-600 hover:text-red-800 disabled:opacity-50" title="Reject">
                            <XCircle size={18} />
                          </button>
                        </div>
                      )}
