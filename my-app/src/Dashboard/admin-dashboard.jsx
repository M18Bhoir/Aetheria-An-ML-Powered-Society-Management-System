...
import {
  Bell, Users, Wrench, Menu, X, Home,
  DollarSign, LogOut, CalendarCheck, Key, ClipboardEdit, Vote,
  FileText, // Expense Logger
  Briefcase // Rentals
} from "lucide-react";
...
const Sidebar = () => {
  ...
  const menu = [
    ...
    { name: "Manage Bookings", icon: <CalendarCheck size={20} />, path: "/admin/manage-bookings" },
    { name: "Guest Requests", icon: <Key size={20} />, path: "/admin/guest-requests" },
    { name: "Manage Polls", icon: <Vote size={20} />, path: "/admin/manage-polls" },
    { name: "Maintenance", icon: <Wrench size={20} />, path: "/admin/maintenance" },
    { name: "Notices", icon: <Bell size={20} />, path: "/admin/notices" },
    { name: "Expense Logger", icon: <FileText size={20} />, path: "/admin/expense-logger" },
    { name: "Rental Applications", icon: <Briefcase size={20} />, path: "/admin/manage-rentals" },
  ];
  ...
  const NavItem = ({ item }) => {
    if (!item.path) {
      return (
        <button
          disabled
          className={`flex items-center space-x-3 w-full p-3 rounded-lg text-gray-500
            ${isOpen ? "px-4" : "justify-center"} cursor-not-allowed`}
